...
function animate() {
  ...
  if (debugHud) {
    ...
    debugHud.textContent =
      ...
      `\nactive: deck ${seratoSync.activeDeck || '?'}  (D to switch)`;
  }
}
...
function onKeyDown(e) {
  switch (e.code) {
    ...
    case 'ArrowLeft': // ←: sensitivity (amplitude) down
      ...
      break;
    case 'Period': // >: reactivity up
      analyzer.reactivity = Math.min(2.0, (analyzer.reactivity || 1.0) + 0.2);
      break;
    case 'Comma': // <: reactivity down
      analyzer.reactivity = Math.max(0.2, (analyzer.reactivity || 1.0) - 0.2);
      break;
...
fetch('http://localhost:3456/api/folders').then(r => r.json()).then(data => {
  const statusEl = document.getElementById('folder-status');
  if (statusEl) statusEl.textContent = `${data.folders.length} folder(s) loaded`;
}).catch(() => {
  const statusEl = document.getElementById('folder-status');
